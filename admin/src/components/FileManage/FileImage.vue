<template>
  <el-row :style="rowStyle">
    <el-col :span="fileSpan" :style="fileStyle">
      <div v-if="fileUrl" class="h-full" :class="avatar ? '':'mr-4 flex justify-center'">
        <el-image
          v-if="fileType === 'image'"
          :style="imageStyle"
          :src="fileUrl"
        ...
        <div v-else-if="fileType === 'word'"  class="flex items-center h-full">
      ...
      <div v-else class="mr-4">
        ...
        <div class="image__default" v-else @click="fileUpload()">
    ...
    <el-col :span="operSpan">
      <el-row>
        ...
        <el-col>
          ...
          <el-text @click="fileDownload()" v-if="fileType !== 'image'" style="max-width: 80%;cursor: pointer;" size="primary" truncated :title="fileUrl">{{ fileUrl }}</el-text>
        </el-col>
      </el-row>
    </el-col>
    <el-dialog
...
const rowStyle = computed(() => {
  ...
  return { width: '100%',border: '1px solid var(--el-border-color)',padding: '15px',backgroundColor: 'var(--el-fill-color-light)',borderRadius: '5px' }
})
...
function fileSubmit(fileList) {
  ...
  if (fileLength) {
    ...
    if (props.dataType === 'file_id') {
      model.value = fileList[index]['file_id']
    }
    if (props.dataType === 'file_url') {
      model.value = fileList[index]['file_url']
    }
