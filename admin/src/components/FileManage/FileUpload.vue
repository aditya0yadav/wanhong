<template>
  <el-row style="width: 100%">
    <el-col :span="12" :style="{ height: height + 'px' }">
      <div>
        <div v-if="file.file_type === 'image'">
          <el-image :style="{ height: height + 'px' }" :src="file.file_url" :fit="fit"
            :preview-src-list="[file.file_url]" title="点击看大图">
            <template #error>
    ...
    <el-dialog v-model="fileDialog" :title="fileTitle ? fileTitle : uploadBtn" :close-on-click-modal="false"
      :close-on-press-escape="false" top="1vh" width="80%" append-to-body>
