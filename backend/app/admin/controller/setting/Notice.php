...
class Notice extends BaseController
{
    ...
    public function push(NoticeModel $NoticeModel,MemberModel $MemberModel,NoticeDataModel $NoticeDataModel){
        $param = $this->params(NoticeService::$edit_field);
        $res = $NoticeModel->where('notice_id',$param['notice_id'])->find();
        if($res->status !=0){
           return error('该通知已推送'); 
        }
        if($res->type == 0){
            $memberIds = $MemberModel->where('is_disable',0)->where('is_delete',0)->column('member_id');
            ...
            $NoticeDataModel->saveAll($data);
            $res->status = 1;
            $res->save();
            return success('推送成功');
        }
        if($res->type == 1){
            $memberIds = explode(',',$res->members);
            $date = date('Y-m-d H:i:s');
            $data = [];
            foreach ($memberIds as $member_id){
                $data[] = [
