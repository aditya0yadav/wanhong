...
use app\common\model\setting\NoticeDataModel;
...
class Member extends BaseController
{
     ...
     * })
     */
    public function notice(NoticeDataModel $NoticeDataModel){
        $member_id = member_id(true);
        $list = $NoticeDataModel->where('member_id',$member_id)->order('notice_data_id desc')->limit(50)->select();
        return success(['list'=>$list]);
    }
    public function unread(NoticeDataModel $NoticeDataModel){
        $member_id = member_id(true);;
        $unread = $NoticeDataModel->where('member_id',$member_id)->where('is_read',0)->count();
        return success($unread);
    }
    public function noticedetail(NoticeDataModel $NoticeDataModel){
        $member_id = member_id(true);
        $param = $this->params([
            'notice_data_id/d' => 0,
        ]);
        $data = $NoticeDataModel->where('member_id',$member_id)->where('notice_data_id',$param['notice_data_id'])->find();
    ...
    public function marknotice(NoticeDataModel $NoticeDataModel){
        $member_id = member_id(true);
        $param = $this->params([
            'notice_data_id/d' => 0,
        ]);
        if($param['notice_data_id'] == -1){
            $NoticeDataModel->where('member_id',$member_id)->update(['is_read'=>1]);
        } else {
            $NoticeDataModel->where('member_id',$member_id)->where('notice_data_id',$param['notice_data_id'])->update(['is_read'=>1]);
        }
        return success();
    }
    public function info()
    {
        ...
        $sevenwhere[] = ['member_id','=',$param['member_id']];
        $sevenwhere[] = ['reward_status','in',[0,1]];
        $sevenwhere[] = ['create_time','between',[$starttime,$endtime]];
        $rewardModel = new RewardModel();
        $sevenData = $rewardModel->where($sevenwhere)->field('DATE_FORMAT(create_time, "%m-%d") as date, ROUND(SUM(member_payout),2) as total_member_payout, ROUND(SUM(member_payout/usd_currency_coins),2) as usd_total_member_payout')
             ->group('date')
            ->select();
        $column = [];
        ...
        $data['seven_data'] = array_fill_keys($column, 0);
        $data['usd_seven_data'] = array_fill_keys($column, 0);
        foreach ($sevenData as $k => $v) {
            $data['seven_data'][$v['date']] = $v['total_member_payout'];
            $data['usd_seven_data'][$v['date']] = $v['usd_total_member_payout'];
        }
        $data['completed_offers'] = $rewardModel->where('member_id','=',$param['member_id'])->where('reward_status','in',[0,1])->count();
        $data['earned'] = round($rewardModel->where('member_id','=',$param['member_id'])->where('reward_status','in',[0,1])->sum('member_payout'),2);
        $data['usd_earned'] = $rewardModel->where('member_id','=',$param['member_id'])->where('reward_status','in',[0,1])->field('round(sum(member_payout/usd_currency_coins),2) as member_payout')->find();
        return success($data);
    }
    // 会员开放信息查询
    public function baseinfo(){
        ...
        $result['member_id'] = $data['member_id'];
        ...
        $result['seven_data'] = array_fill_keys($column, 0);
        $result['usd_seven_data'] = array_fill_keys($column, 0);
        foreach ($sevenData as $k => $v) {
            $result['seven_data'][$v['date']] = $v['total_member_payout'];
            $result['usd_seven_data'][$v['date']] = $v['usd_total_member_payout'];
        }
        $result['completed_offers'] = $rewardModel->where('member_id','=',$param['member_id'])->where('reward_status','in',[0,1])->count();
        $result['earned'] = $rewardModel->where('member_id','=',$param['member_id'])->where('reward_status','in',[0,1])->sum('member_payout');
        $result['usd_earned'] = $rewardModel->where('member_id','=',$param['member_id'])->where('reward_status','in',[0,1])->field('round(sum(member_payout/usd_currency_coins),2) as member_payout')->find();
        $result['rewards'] = $rewardModel->with(['platform' => function($query){
            return $query->field('platform_id,platform_name');
        }])->where('member_id','=',$param['member_id'])->where('reward_status','in',[0,1])->field('platform_id,project_pno,member_payout,usd_currency_coins,create_time')->limit(5)->order('reward_id desc')->select();
        return success($result);
    }
    /**
     * @Apidoc\Title("修改昵称")
     * @Apidoc\Method("POST")
     * @Apidoc\Param(ref="app\common\model\member\MemberModel", field="nickname")
     */
    public function changeNickname()
    {
        ...
        validate(MemberValidate::class)->scene('editnickname')->check($param);
    ...
    public function edit(){
        ...
        $pwd = $this->params(['password/s' => '']);
        if($pwd['password']){
            $param['password'] = $pwd['password'];
        }
        $teamId = member_team_id(true);
        validate(MemberValidate::class)->scene('apiedit')->check($param);
        $memberModel = new MemberModel();
        $member = $memberModel->where('member_id',$param['member_id'])->where('team_id',$teamId)->find();
        if(empty($member)){
            return error('用户不存在或不属于您的团队');
        } else {
            $data = MemberService::edit($param['member_id'], $param);
            return success();
        }
    ...
    public function add()
    {
        ...
        $param['group_ids'] = [1];
        $param['team_id'] = member_team_id(true);
        $memberModel = new MemberModel();
        $teamModel = new TeamModel();
        $memberCount = $memberModel->where('team_id',$param['team_id'])->count();
        $team = $teamModel->where('team_id',$param['team_id'])->find();
        if($memberCount >= $team['auth_num']){
            return error('团队用户数量已达上限');
        }
        $param['platform']    = SettingService::PLATFORM_YA;
        $param['application'] = SettingService::APP_YA_ADMIN;
        validate(MemberValidate::class)->scene('apiadd')->check($param);
    ...
    public function list(){
        ...
        $where[] = ['group_ids','between',[1,1]];
        $data = MemberService::list($where, $this->page(), $this->limit(), $this->order(),'avatar_id,rate,username,nickname,login_ip,login_time,is_disable');
        return success($data);
    }
    /**
     * @Apidoc\Title("更换头像")
