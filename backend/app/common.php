 ...
 * @return int
 */
function member_team_id($exce = false)
{
    $api_token = api_token();

    return MemberTokenService::memberTeamId($api_token, $exce);
}
...
// 加密函数
function encrypt($plaintext, $key='kF3ZjCwJjF4zSFj8') {
    ...
    $encrypt = base64_encode($iv . $ciphertext);
    // 转义Z为ZZ
    $encrypt = str_replace('Z', 'ZZ', $encrypt);
    // 替换特殊字符
    $encrypt = str_replace('+', 'Za', $encrypt);
    $encrypt = str_replace('/', 'Zb', $encrypt);
    $encrypt = str_replace('=', 'Zc', $encrypt);
    return $encrypt;
}
// 解密函数
function decrypt($ciphertext, $key='kF3ZjCwJjF4zSFj8') {
    // 替换回特殊字符
    $ciphertext = str_replace(['Za', 'Zb', 'Zc'], ['+', '/', '='], $ciphertext);
    $ciphertext = str_replace('ZZ', 'Z', $ciphertext);
    $data = base64_decode($ciphertext);
    $ivLength = openssl_cipher_iv_length('aes-256-cbc');
    $iv = substr($data, 0, $ivLength);
    $rawCiphertext = substr($data, $ivLength);
