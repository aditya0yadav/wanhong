...
class MemberService
{
    ...
    public static function list($where = [], $page = 1, $limit = 10, $order = [], $field = '', $total = true)
    {
        ...
        if (empty($field)) {
            $field = $group . ',avatar_id,nickname,username,phone,email,sort,is_super,is_disable,create_time,login_ip,login_time';
        } else {
            $field = $group . ',' . $field;
        }
        if (empty($order)) {
    ...
    public static function info($id, $exce = true, $group = false, $third = false)
    {
        ...
        if (empty($info)) {
            ...
            $team = TeamService::info($info['team_id']);
    ...
    public static function login($param, $type = '')
    {
        ...
        $field = $pk . ',team_id,username,nickname,phone,email,password,login_num,is_disable';
        ...
        if(empty($member['team_id'])){
           exception('账号无归属团队,禁止登录'); 
        } else {
            $team = TeamService::info($member['team_id']);
            if(empty($team)){
                exception('您的团队不存在,禁止登录'); 
            } else {
                if($team['is_disable']){
                    exception('团队已禁用,禁止登录');
                }
            }
        }
    ...
    public static function loginField($member)
    {
        ...
        $fields = ['member_id', 'avatar_uid', 'avatar_url', 'nickname', 'username', 'login_ip', 'login_time', 'login_num','team_name'];
