...
use app\common\model\topic\TopicAnserModel;
/**
 * 人设
 */
class TopicService
{
    ...
    public static function list($where = [], $page = 1, $limit = 10,  $order = [], $field = '', $total = true)
    {
        ...
        if (empty($field)) {
            $field = $group . ',platform_id,topic_name,topic_params,create_time,update_time,sort';
        } else {
            $field = $group . ',' . $field;
        }
        if (empty($order)) {
        ...
        $list = $model->with('platform')->field($field)->where($where)->append($append)->order($order)->select()->toArray();
    ...
    public static function add($param)
    {
        ...
        $param['topic_params'] = implode(',',$param['topic_params']);
        // 启动事务
        $model->startTrans();
        try {
            // 添加
