...
class MemberValidate extends Validate
{
    ...
    protected $rule = [
        'ids'          => ['require', 'array'],
        'team_id'    => ['require','checkTeamIsExist'],
        'member_id'    => ['require'],
        'nickname'     => ['length' => '1,5'],
    ...
    protected $message = [
        ...
        'rate.require'  => '请输入抽佣比例',
        'rate.between'  => '抽佣比例限制0-100',
    ...
    protected $scene = [
        ...
        'apiadd'               => ['nickname', 'username','rate'],
        'apiedit'              => ['member_id', 'nickname', 'username','rate'],
        'editnickname'              => ['member_id', 'nickname'],
        'dele'                 => ['ids'],
        'region'               => ['ids'],
        'edittag'              => ['ids'],
        'editgroup'            => ['ids'],
    ...
    protected function sceneApiadd()
    {
        return $this->only(['nickname', 'username', 'password','rate'])
            ->append('username', ['checkUsernameExisted']);
    }
    // 验证场景定义：后台修改
    protected function sceneApiedit()
    {
        return $this->only(['member_id', 'nickname', 'username','rate'])
            ->append('username', ['checkUsernameExisted']);
    }
    ...
    // 自定义验证规则：团队是否存在
    protected function checkTeamIsExist($value, $rule, $data = [])
    {
        $model = new TeamModel();
